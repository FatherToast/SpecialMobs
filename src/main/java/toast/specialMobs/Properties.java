...
public abstract class Properties {
    ...
    private static final boolean[] monsterVanilla = new boolean[_SpecialMobs.MONSTER_KEY.length];
    /// Property array, matched up to MONSTER_KEY[], special monster species will only spawn if the same index here is true.
    private static final boolean[] monsterSpawn = new boolean[_SpecialMobs.MONSTER_KEY.length];
    ...
    private static final int[][] monsterWeights = new int[_SpecialMobs.MONSTER_KEY.length][];
    ...
    private static void loadIntSet( Configuration config, String category, String field, String defaultValue, String comment, HashSet<Integer> intSet ) {
        ...
        if( !intArray[0].trim().isEmpty() ) {
            ...
            for( String s : intArray ) {
                try {
                    intSet.add( Integer.parseInt( s.trim() ) );
                }
                catch( Exception ex ) {
                    throw new RuntimeException( "Invalid dimension blacklist entry: " + s.trim(), ex );
                }
            }
        }
    }
