...
public class DataWatcherHelper {
    ...
    public static class EntityData {
        ...
        protected void generateNeededKeys( int newKeys ) {
            ...
            if( autoKeys != null ) {
                int index = 0;
                for( byte key = 0; index < autoKeys.length && key < DataWatcherHelper.MAX_KEYS; key++ ) {
                    if( isKeyUnused( key ) ) {
                        newKeys = addKey( newKeys, key );
                        autoKeys[index++] = key;
                    }
                }
                if( index < autoKeys.length ) {
                    _SpecialMobs.debugException( "Not enough data watcher keys available for " + id + "! (" + (autoKeys.length - index) + " more needed)" );
                }
                keysUsed |= newKeys; // Add keys just generated
            }
        ...
        protected void verifyNeededKeys() {
            if( autoKeys != null && autoKeyIndex < autoKeys.length ) {
                _SpecialMobs.debugException( "Too many data watcher keys requested for " + id + "! (" + (autoKeys.length - autoKeyIndex) + " extras)" );
            }
