...
public class EntityThiefEnderman extends Entity_SpecialEnderman {
    ...
    public EntityThiefEnderman( World world ) {
        super( world );
        getSpecialData().setTextures( EntityThiefEnderman.TEXTURES );
        experienceValue += 1;
    }
    ...
    protected void onTypeAttack( Entity target ) {
        if( target instanceof EntityLivingBase ) {
            for( int i = 64; i-- > 0; )
                if( teleportTarget( (EntityLivingBase) target ) ) {
                    break;
                }
        }
        for( int i = 64; i-- > 0; )
    ...
    private boolean teleportTarget( EntityLivingBase target ) {
        ...
        target.posX += (rand.nextDouble() - 0.5) * 32.0;
        target.posY += rand.nextInt( 32 ) - 16;
        target.posZ += (rand.nextDouble() - 0.5) * 32.0;
