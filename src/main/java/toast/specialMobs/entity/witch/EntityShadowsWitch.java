...
public class EntityShadowsWitch extends Entity_SpecialWitch {
    ...
    public EntityShadowsWitch( World world ) {
        super( world );
        getSpecialData().setTextures( EntityShadowsWitch.TEXTURES );
        getSpecialData().immuneToPotions.add( Potion.blindness.id );
    }
    
    /// Called every tick while this entity is alive.
    @Override
    public void onLivingUpdate() {
        if( !worldObj.isRemote && isEntityAlive() && getAttackTarget() != null && rand.nextInt( 10 ) == 0 && getAttackTarget().isPotionActive( Potion.blindness ) ) {
            getAttackTarget().removePotionEffect( Potion.nightVision.id );
        }
    ...
    protected EntityPotion adjustSplashPotion( EntityPotion thrownPotion, EntityLivingBase target, float range, float distance ) {
        ...
        if( target.getHealth() >= 4.0F && !target.isPotionActive( Potion.blindness ) )
            return new EntityPotion( worldObj, this, makeShadowPotion() );
    ...
    protected void dropRareDrop( int superRare ) {
        entityDropItem( makeShadowPotion(), 0.0F );
    }
