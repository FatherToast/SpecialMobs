...
public class EntityUndeadWitch extends Entity_SpecialWitch {
    ...
    public EntityUndeadWitch( World world ) {
        super( world );
        getSpecialData().setTextures( EntityUndeadWitch.TEXTURES );
        skeletonCount = (byte) (rand.nextInt( 3 ) + 3);
    }
    
    /// Overridden to modify inherited attributes.
    @Override
    public void adjustTypeAttributes() {
        getSpecialData().multAttribute( SharedMonsterAttributes.movementSpeed, 1.1 );
        
        setCurrentItemOrArmor( 0, new ItemStack( Items.bone ) );
    }
    ...
    public void tryDrinkPotion() {
        if( potionThrowDelay <= 0 ) {
            ...
            else if( rand.nextFloat() < 0.025F && getHealth() < getMaxHealth() ) {
                drinkPotion( 16396 ); // Splash Instant Damage
            }
            else if( rand.nextFloat() < 0.025F && getHealth() < getMaxHealth() ) {
                drinkPotion( 16428 ); // Splash Instant Damage II
            }
    ...
    public void attackEntityWithRangedAttack( EntityLivingBase target, float range ) {
        if( skeletonCount > 0 && rand.nextInt( 4 ) == 0 ) {
            skeletonCount--;
            Entity_SpecialSkeleton skeleton = new Entity_SpecialSkeleton( worldObj );
            ...
            skeleton.setCurrentItemOrArmor( 4, new ItemStack( Items.leather_helmet, 1, Items.leather_helmet.getMaxDamage() - 2 - rand.nextInt( 5 ) ) );
            
            skeleton.worldObj.spawnEntityInWorld( skeleton );
            worldObj.playSoundAtEntity( skeleton, "mob.ghast.fireball", 0.5F, 2.0F / (rand.nextFloat() * 0.4F + 0.8F) );
        }
        else {
    ...
    public void writeEntityToNBT( NBTTagCompound tag ) {
        ...
        saveTag.setByte( "Skeletons", skeletonCount );
    }
    ...
    public void readEntityFromNBT( NBTTagCompound tag ) {
        ...
        if( saveTag.hasKey( "Skeletons" ) ) {
            skeletonCount = saveTag.getByte( "Skeletons" );
        }
        else if( tag.hasKey( "Skeletons" ) ) {
            skeletonCount = tag.getByte( "Skeletons" );
        }
    }
}