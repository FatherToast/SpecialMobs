...
public class EntityDominationWitch extends Entity_SpecialWitch {
    ...
    public EntityDominationWitch( World world ) {
        super( world );
        getSpecialData().setTextures( EntityDominationWitch.TEXTURES );
    }
    ...
    public void adjustTypeAttributes() {
        getSpecialData().multAttribute( SharedMonsterAttributes.movementSpeed, 0.8 );
    }
    ...
    public void onLivingUpdate() {
        if( !worldObj.isRemote && isEntityAlive() && pullDelay-- <= 0 && getAttackTarget() != null && rand.nextInt( 20 ) == 0 ) {
            EntityLivingBase target = getAttackTarget();
            double distanceSq = target.getDistanceSqToEntity( this );
            
            if( distanceSq > 100.0 && distanceSq < 196.0 && EntityDominationWitch.canAffectMind( target ) && canEntityBeSeen( target ) ) {
                pullDelay = 80;
                
                double vX = posX - target.posX;
                double vY = posY - target.posY;
    ...
    protected void dropRareDrop( int superRare ) {
        ...
        if( superRare > 0 ) {
            ...
            damage = (int) (0.6F * damage + 0.3F * rand.nextInt( damage ));
        }
        ...
        drop.addEnchantment( Enchantment.unbreaking, rand.nextInt( 3 ) + 1 );
