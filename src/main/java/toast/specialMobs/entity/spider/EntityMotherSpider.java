...
public class EntityMotherSpider extends Entity_SpecialSpider {
    ...
    public EntityMotherSpider( World world ) {
        super( world );
        setSize( 1.8F, 1.2F );
        getSpecialData().setTextures( EntityMotherSpider.TEXTURES );
    ...
    protected void dropFewItems( boolean hit, int looting ) {
        ...
        if( hit && (rand.nextInt( 3 ) == 0 || rand.nextInt( 1 + looting ) > 0) ) {
            entityDropItem( new ItemStack( Items.spawn_egg, 1, EntityList.getEntityID( new EntitySpider( worldObj ) ) ), 0.0F );
        }
