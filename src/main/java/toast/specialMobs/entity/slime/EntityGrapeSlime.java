...
public class EntityGrapeSlime extends Entity_SpecialSlime {
    ...
    public EntityGrapeSlime( World world ) {
        super( world );
        getSpecialData().setTextures( EntityGrapeSlime.TEXTURES );
    ...
    protected boolean jumpByType( EntityPlayer target ) {
        if( target != null ) {
            ...
            if( distance > 36.0F && distance < 144.0F ) {
                ...
                motionX = vX / vH * 1.11 + motionX * 0.21;
                motionY = 0.41 * 2.61;
                motionZ = vZ / vH * 1.11 + motionZ * 0.21;
                onGround = false;
                return true;
            }
        }
    ...
    protected void dropFewItems( boolean hit, int looting ) {
        ...
        if( getSlimeSize() == 1 ) {
            ...
            if( hit && (rand.nextInt( 3 ) == 0 || rand.nextInt( 1 + looting ) > 0) ) {
                entityDropItem( new ItemStack( Items.dye, 1, 5 ), 0.0F );
            }
        }
    }
