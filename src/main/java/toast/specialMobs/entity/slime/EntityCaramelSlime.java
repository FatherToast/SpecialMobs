...
public class EntityCaramelSlime extends Entity_SpecialSlime {
    ...
    public EntityCaramelSlime( World world ) {
        super( world );
        getSpecialData().setTextures( EntityCaramelSlime.TEXTURES );
    }
    ...
    public void onLivingUpdate() {
        ...
        if( riddenByEntity != null ) {
            riddenByEntity.attackEntityFrom( DamageSource.causeThornsDamage( this ), 1.0F );
            attackTime = 20;
        }
    }
    ...
    protected void onTypeAttack( Entity target ) {
        if( riddenByEntity == null || target.ridingEntity instanceof EntityCaramelSlime ) {
    ...
    protected void dropFewItems( boolean hit, int looting ) {
        ...
        if( getSlimeSize() == 1 ) {
            ...
            if( hit && (rand.nextInt( 3 ) == 0 || rand.nextInt( 1 + looting ) > 0) ) {
                entityDropItem( new ItemStack( Items.dye, 1, 14 ), 0.0F );
            }
        }
    }
