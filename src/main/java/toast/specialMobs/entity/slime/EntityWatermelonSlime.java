...
public class EntityWatermelonSlime extends Entity_SpecialSlime {
    ...
    public EntityWatermelonSlime( World world ) {
        super( world );
        getSpecialData().setTextures( EntityWatermelonSlime.TEXTURES );
    ...
    protected boolean jumpByType( EntityPlayer target ) {
        if( target != null ) {
            float distance = (float) getDistanceSqToEntity( target );
            if( distance < 16.0F ) {
                double vX = target.posX - posX;
                double vZ = target.posZ - posZ;
                double vH = Math.sqrt( vX * vX + vZ * vZ );
                motionX = vX / vH * 1.41 + motionX * 0.2;
                motionY = 0.45;
                motionZ = vZ / vH * 1.41 + motionZ * 0.2;
                onGround = false;
                slimeJumpDelay /= 3;
                return true;
            }
    ...
    protected void onTypeAttack( Entity target ) {
        target.addVelocity( -MathHelper.sin( rotationYaw * (float) Math.PI / 180.0F ) * 0.8F, 0.1, MathHelper.cos( rotationYaw * (float) Math.PI / 180.0F ) * 0.8F );
        motionX *= -0.4;
        motionZ *= -0.4;
    }
    ...
    protected void dropFewItems( boolean hit, int looting ) {
        ...
        if( getSlimeSize() == 1 ) {
            for( int i = rand.nextInt( 3 + looting ); i-- > 0; ) {
                dropItem( Items.melon, 1 );
            }
            if( hit && (rand.nextInt( 3 ) == 0 || rand.nextInt( 1 + looting ) > 0) ) {
                entityDropItem( new ItemStack( Items.dye, 1, rand.nextBoolean() ? 9 : 10 ), 0.0F );
            }
            if( hit && (rand.nextInt( 3 ) == 0 || rand.nextInt( 1 + looting ) > 0) ) {
                dropItem( Items.speckled_melon, 1 );
            }
        }
    }
