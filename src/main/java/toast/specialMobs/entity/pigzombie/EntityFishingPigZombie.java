...
public class EntityFishingPigZombie extends Entity_SpecialPigZombie implements IAngler {
    ...
    protected void entityInit() {
        super.entityInit();
        /// heldItem; (boolean) If this is not true, the heldItem is rendered as a stick.
        dataWatcher.addObject( EntityFishingPigZombie.DW_FISHING_ROD, (byte) 1 );
    }
    ...
    public void setFishingRod( boolean rod ) {
        dataWatcher.updateObject( EntityFishingPigZombie.DW_FISHING_ROD, rod ? (byte) 1 : (byte) 0 );
    }
    
    public boolean getFishingRod() {
        return dataWatcher.getWatchableObjectByte( EntityFishingPigZombie.DW_FISHING_ROD ) == 1;
    }
    
    /// Returns the heldItem.
    @Override
    public ItemStack getHeldItem() {
        if( worldObj.isRemote && !getFishingRod() )
