...
public
class Entity_SpecialPigZombie extends EntityPigZombie implements ISpecialMob, IRangedAttackMob
{
	...
	private static final DataParameter< Float > SCALE = EntityDataManager.createKey( Entity_SpecialPigZombie.class, DataSerializers.FLOAT );
	...
	{
		return SpecialMobsMod.TEXTURE_PATH + "pigzombie/" + type + ".png";
	}
	
	static
	void ADD_BASE_LOOT( LootTableBuilder loot )
	{
		loot.addLootTable( "main", "Base loot", LootTableList.ENTITIES_ZOMBIE_PIGMAN );
	}
	...
	public
	Entity_SpecialPigZombie( World world )
	{
		super( world );
	...
	void setEquipmentBasedOnDifficulty( DifficultyInstance difficulty )
	{
		...
		if( rand.nextFloat( ) < Config.get( ).ZOMBIE_PIGMEN.BOW_CHANCE ) {
	...
	IEntityLivingData onInitialSpawn( DifficultyInstance difficulty, @Nullable IEntityLivingData data )
	{
		...
		if( isChild( ) ) {
			if( world.rand.nextFloat( ) < Config.get( ).ZOMBIE_PIGMEN.CHICKEN_JOCKEY_CHANCE ) {
			...
			else if( world.rand.nextFloat( ) < Config.get( ).ZOMBIE_PIGMEN.CHICKEN_JOCKEY_FORCED_CHANCE ) {
