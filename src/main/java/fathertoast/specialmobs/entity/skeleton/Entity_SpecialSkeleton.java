...
public
class Entity_SpecialSkeleton extends EntitySkeleton implements ISpecialMob
{
	...
	private static final DataParameter< Float >   SCALE    = EntityDataManager.createKey( Entity_SpecialSkeleton.class, DataSerializers.FLOAT );
	private static final DataParameter< Boolean > IS_CHILD = EntityDataManager.createKey( Entity_SpecialSkeleton.class, DataSerializers.BOOLEAN );
	...
	{
		return SpecialMobsMod.TEXTURE_PATH + "skeleton/" + type + ".png";
	}
	
	static
	void ADD_BASE_LOOT( LootTableBuilder loot )
	{
		loot.addLootTable( "main", "Base loot", LootTableList.ENTITIES_SKELETON );
	}
	...
	public
	Entity_SpecialSkeleton( World world )
	{
		super( world );
	...
	protected
	void setCollisionSize( float multiplier ) { setSize( 0.6F * multiplier, 1.99F * multiplier ); }
	...
	void setEquipmentBasedOnDifficulty( DifficultyInstance difficulty )
	{
		...
		// Equip a melee weapon if this skeleton does not roll as a bow user
		// No need to equip a bow otherwise, as it already has one
		if( rand.nextFloat( ) >= Config.get( ).SKELETONS.BOW_CHANCE ) {
	...
	IEntityLivingData onInitialSpawn( DifficultyInstance difficulty, @Nullable IEntityLivingData data )
	{
		...
		setChild( rand.nextFloat( ) < Config.get( ).SKELETONS.BABY_CHANCE );
