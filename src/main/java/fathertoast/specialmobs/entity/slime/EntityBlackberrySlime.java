...
public
class EntityBlackberrySlime extends Entity_SpecialSlime
{
	...
	private static final ResourceLocation[] TEXTURES = {
		new ResourceLocation( GET_TEXTURE_PATH( "blackberry" ) )
	};
	...
	void BUILD_LOOT_TABLE( LootTableBuilder loot )
	{
		...
		loot.addUncommonDrop( "uncommon", "Slime color", new ItemStack( Items.DYE, 1, EnumDyeColor.BLACK.getDyeDamage( ) ) );
	}
	
	private int fuseTime = 0;
	
	public
	EntityBlackberrySlime( World world ) { super( world ); }
	
	@Override
	protected
	EntitySlime getSplitSlime( ) { return new EntityBlackberrySlime( world ); }
		...
		if( isEntityAlive( ) ) {
			...
			if( !world.isRemote && target != null && getDistanceSq( target ) < 9.0F + (getSlimeSize( ) - 1.0F) * 2.0F ) {
				if( fuseTime == 0 ) {
					...
					dead = true;
					world.createExplosion( this, posX, posY, posZ, getSlimeSize( ) + 0.5F, griefing );
					setDead( );
				}
				fuseTime++;
